...
def _phase_instruction(phase: Phase, run: RunState, repo_root: Path,
    ...
    base = {
        ...
        "design": (
            ...
            f"  STORY: {story}\n"
            f"BUDGET YOUR TURNS. You have a limited number of turns and the run "
            f"FAILS if {design_file} does not exist when they run out — a phase that "
            f"explored thoroughly and wrote nothing has produced nothing. Read no "
            f"more than about ten files, then write. If you are unsure whether to "
            f"read one more file or write the design, WRITE THE DESIGN: an "
            f"incomplete design that exists can be improved, one that was never "
            f"written cannot.\n"
            f"Read READ-ONLY, and only what you need: the classes and interfaces "
            f"this change touches, the closest existing precedent, and anything the "
            f"change must not break. Do NOT trace the full downstream call graph or "
            f"survey the whole repository — a design written from the story alone is "
            f"a guess, but an exhaustive survey is not the job either.\n"
            f"Then write the technical design to this EXACT file (its parent "
            f".harness ALREADY EXISTS, do not mkdir, shell is disallowed): "
            f"{design_file}\n"
            f"Write the file in ONE pass, complete, rather than creating it empty "
            f"and refining it across several turns.\n"
